...
class TempCXR(nn.Module):
    ...
    def forward(self, curr_imgs, prev_imgs, static_texts, dynamic_texts):
        """
        Returns:
            vs : static image embedding   (B, 128)
            ts : static text embedding    (B, 128)
            vd : dynamic image embedding  (B, 128)
            td : dynamic text embedding   (B, 128)
        """
        ...
        # DYNAMIC BRANCH
        # =====================================================
        _, prev_patches = self.image_encoder(prev_imgs)
        curr_patches = curr_patches.detach()
        prev_patches = prev_patches.detach()
